...
export async function GET(req: NextRequest) {
  try {
    ...
    // Strategy: show earners where users.affiliate_earnings > 0
    ...
    const usersQuery = supabase
      .from("users")
      .select("id, username, full_name, user_type, affiliate_earnings");
    const { data: users, error: usersErr } = includeZeros
      ? await usersQuery
      : await usersQuery.gt("affiliate_earnings", 0);
    if (usersErr)
    ...
    const creators = (users || [])
      .filter((u) => u.user_type === "creator")
      .map((u) => u.id);
    const advertisers = (users || [])
      .filter((u) => u.user_type === "advertiser")
      .map((u) => u.id);
    ...
    for (const p of creatorProfiles.data || [])
      creatorBal[p.id] = p.withdrawable_balance || 0;
    const advertiserBal: Record<string, number> = {};
    for (const p of advertiserProfiles.data || [])
      advertiserBal[p.id] = p.withdrawable_balance || 0;
    ...
    for (const t of lastTxns || []) {
      ...
      if (!lastAtByUser[uid])
        lastAtByUser[uid] = (t as any).created_at as string;
    }
    ...
    const items = (users || []).map((u) => {
      const bal =
        u.user_type === "creator"
          ? creatorBal[u.id] || 0
          : advertiserBal[u.id] || 0;
      const totalOther = (u as any).affiliate_earnings || 0;
      return {
        user_id: u.id,
        username: u.username,
