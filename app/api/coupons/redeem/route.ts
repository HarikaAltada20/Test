...
function loadCoupons(): Record<string, CouponConfig> {
  try {
    ...
    if (raw) {
      ...
      if (parsed && typeof parsed === 'object') return parsed as Record<string, CouponConfig>;
    }
  } catch (e) {
  ...
  // Defaults
  return {
    DISCORD10: { audience: 'creator', reward: { cash_cents: 10 }, remarks: 'coupon:DISCORD10', unique: true },
  };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: userRow, error: userErr } = await supabase
      ...
      .eq('id', user.id)
      .single();
    if (userErr || !userRow) return NextResponse.json({ error: 'User not found' }, { status: 400 });

    const audience = config.audience || 'any';
    if (audience !== 'any' && userRow.user_type !== audience) {
      return NextResponse.json({ error: 'Not eligible for this coupon' }, { status: 403 });
    }
    ...
    if (config.unique !== false) {
      ...
      if (redemptionErr) {
        ...
        return NextResponse.json({ success: true, message: 'Coupon already redeemed' });
      }
    }
    ...
    if (config.reward.cash_cents && config.reward.cash_cents > 0) {
      ...
      if (userRow.user_type === 'creator') {
        const { data: creditRes, error: rpcErr } = await supabase
          .rpc('credit_creator_cash_atomic', { p_user_id: user.id, p_amount_cents: cashCredited, p_description: description, p_remarks: remarks });
        if (rpcErr || !creditRes) return NextResponse.json({ error: 'Failed to credit cash' }, { status: 500 });
      } else {
        const { data: creditRes, error: rpcErr } = await supabase
          .rpc('credit_advertiser_cash_atomic', { p_user_id: user.id, p_amount_cents: cashCredited, p_description: description, p_remarks: remarks });
        if (rpcErr || !creditRes) return NextResponse.json({ error: 'Failed to credit cash' }, { status: 500 });
      }
    }
    ...
    if (config.reward.coins && config.reward.coins > 0) {
      ...
      const { error: userUpdateErr } = await supabase
        ...
        .update({
          ...
          total_lifetime_coins_earned: (userRow.total_lifetime_coins_earned || 0) + coinsCredited,
        })
        .eq('id', user.id);
      if (userUpdateErr) return NextResponse.json({ error: 'Failed to credit coins' }, { status: 500 });
    ...
    return NextResponse.json({ success: true, cash_cents: cashCredited, coins: coinsCredited });
  } catch (e: any) {
