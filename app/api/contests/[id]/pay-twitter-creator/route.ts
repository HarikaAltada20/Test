...
export async function POST(
  ...
  try {
    ...
    const { creatorId, amountInCents, isCustom, paymentProofUrl, paymentDescription, customRemarks } = await request.json();
    ...
    // Get contest to verify it's a Twitter leaderboard contest
    const { data: contest, error: contestError } = await supabase
      .from("contests")
      .select("id, title, advertiser_id, platform, contest_type, contest_based_details, post_contest_status")
      .eq("id", contestId)
      .single();
    ...
    // Verify contest type is leaderboard
    if (contest.contest_type !== "leaderboard") {
      return NextResponse.json(
        { error: "This endpoint is only for leaderboard contests" },
        { status: 400 }
      );
    }
    ...
    const { data: leaderboardEntry, error: leaderboardError } = await supabaseAdmin
      ...
      .select("id, creator_id, current_rank, total_points, paid, earnings, paid_rank")
    ...
    if (customAmount > 0) {
      ...
      const prizeForRank = prizes.find((p: any) => p.position === leaderboardEntry.current_rank);
      if (!prizeForRank) {
        return NextResponse.json(
    ...
    const { data: rewardInThisCycle } = await supabaseAdmin
      ...
      .eq("user_id", creatorId)
      .eq("type", "reward")
      .contains("metadata", { contest_id: contestId, twitter_creator_id: creatorId, payout_cycle: nextCycle });
    ...
    const creditRes = await creditCreatorWithdrawableBalance(
      ...
      {
        remarks: customRemarks || (customAmount > 0 ? "Custom Twitter payout credited to creator wallet" : "Standard Twitter payout credited to creator wallet"),
        metadata: {
          contest_id: contestId,
          twitter_creator_id: creatorId,
          payout_type: customAmount > 0 ? "custom" : "standard",
    ...
    if (updateError) {
      console.error("[pay-twitter-creator] Error updating leaderboard:", updateError);
      return NextResponse.json(
