...
export async function GET(
  ...
  if (!contestId) {
    return NextResponse.json(
      { error: "Contest ID is required or missing in URL" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (submissionError) {
      console.error(
        `Error fetching user's submissions for contest ${contestId} and user ${user.id}:`,
        submissionError,
      );
      throw new Error(
        `Failed to fetch user submissions: ${submissionError.message}`,
      );
    }
    ...
    if (snapshotError) {
      console.error(
        "[my-submission] contest_my_leaderboard_snapshot:",
        snapshotError,
      );
      throw new Error(
    ...
    const creator_wise_total_earnings =
      cw != null && typeof cw.total_earnings === "number"
        ? cw.total_earnings
        : null;
    const creator_wise_rank =
      cw != null && typeof cw.creator_rank === "number"
        ? cw.creator_rank
        : null;
    ...
    if (userProfileError) {
      console.warn(
        `Warning: Could not fetch user profile for ${user.id}: ${userProfileError.message}`,
      );
    }
    ...
    if (creatorProfileError) {
      console.warn(
        `Warning: Could not fetch creator profile for ${user.id}: ${creatorProfileError.message}`,
      );
    }
    ...
    if (creatorProfile && mySubmission.platform) {
      if (
        mySubmission.platform === "youtube" &&
        creatorProfile.youtube_account
      ) {
        creator_pfp_url =
          creatorProfile.youtube_account.channel_thumbnail || null;
      } else if (
        mySubmission.platform === "instagram" &&
        creatorProfile.instagram_account
      ) {
        creator_pfp_url =
          creatorProfile.instagram_account.profile_picture_url || null;
      } else if (
        mySubmission.platform === "tiktok" &&
        (creatorProfile as any).tiktok_account
      ) {
        creator_pfp_url =
          (creatorProfile as any).tiktok_account.avatar_url || null;
      }
    }
    ...
    console.error(
      `Error in /my-submission endpoint for contest ${contestId}:`,
      error,
    );
    return NextResponse.json(
      {
