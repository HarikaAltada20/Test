...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      `${origin}/auth/signin?error=oauth_error&message=${encodeURIComponent(
        error
      )}`
    );
  }
  ...
  if (code) {
    ...
    try {
      ...
      if (exchangeError) {
        ...
        return NextResponse.redirect(
          `${origin}/auth/signin?error=auth_code_error&message=${encodeURIComponent(
      ...
      if (!data.user) {
        ...
        return NextResponse.redirect(
          `${origin}/auth/signin?error=no_user_data`
        );
      }
      ...
      if (profileCheckError && profileCheckError.code !== "PGRST116") {
        ...
        return NextResponse.redirect(
          `${origin}/auth/signin?error=profile_check_error`
        );
      }
      ...
      if (!userProfile) {
        console.log(
          "Google OAuth: Creating new user profile for first-time user:",
          user.id
        );
        ...
        if (insertError) {
          ...
          return NextResponse.redirect(
            `${origin}/auth/signin?error=profile_creation_error&message=${encodeURIComponent(
              insertError.message
            )}`
          );
        }
      ...
      return NextResponse.redirect(
        `${origin}/auth/signin?error=callback_error&message=${encodeURIComponent(
          error.message
        )}`
      );
    }
