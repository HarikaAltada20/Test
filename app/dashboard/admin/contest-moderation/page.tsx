...
export default async function ContestModerationPage() {
  ...
  if (!isAdmin) {
    console.log(
      "Non-admin user attempted to access contest moderation:",
      error
    );
    redirect("/dashboard");
  }
  ...
  try {
    // Fetch contests for pending_approval status (default tab) initially
    ...
    const formattedContests = (contestsData || []).map((contest) => ({
      ...
      advertiser_name:
        contest.advertiser_profiles?.company_name || "Unknown Brand",
