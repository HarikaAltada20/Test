...
function UserPickerDialog({
  ...
  const [typeFilter, setTypeFilter] = useState<"all" | "creator" | "advertiser">("all");
      ...
      <div className="relative w-full max-w-2xl mx-4 bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-slate-900">Select Recipients from Users</h3>
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 shrink-0 flex items-center justify-between gap-3">
          <p className="text-sm text-slate-600">
            {picked.size > 0 ? (
              <span>
                <span className="font-semibold text-indigo-600">{picked.size}</span>{" "}
...
export default function WarmUpManualSendModal({
  ...
  const [fromEmailMode, setFromEmailMode] = useState<"account" | "sender" | "custom">("sender");
  ...
  useEffect(() => {
    if (isOpen) {
      const activeProjId = projectId || (projects.length > 0 ? projects[0].id : "");
  ...
  }, [isOpen, projectId, prefillEmails, projects, loadData]);

  useEffect(() => {
    if (isOpen && modalAccounts.length > 0) {
      if (!selectedAccountId) {
        ...
        if (prefillAccountId && modalAccounts.some((a) => a.id === prefillAccountId)) {
          setSelectedAccountId(prefillAccountId);
        } else {
  ...
  const parsedCustomEmails = customRecipients
    .split("\n")
    .map((e) => e.trim())
    .filter((e) => e.includes("@"));
      ...
      const res = await fetch(
        `/api/admin/warm-up/emails/${selectedAccountId}/send-manual`,
        {
          ...
          body: JSON.stringify({
            ...
            from_email: resolvedFromEmail !== selectedAccount?.email
              ? resolvedFromEmail
              : undefined,
          }),
        },
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-600 text-sm mt-0.5">
                  Send warm-up emails manually with custom templates and recipients
                </p>
              </div>
            </div>
            <Button
            ...
            <div className="space-y-5">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label className="text-sm font-semibold">Email Project *</Label>
                    ...
                    selectedAccount.warm_up_status !== "active" && (
                      <div className="mt-3 flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-md">
                        ...
                        <p className="text-xs text-amber-800">
                          This account is <strong>{selectedAccount.warm_up_status}</strong>. Sending will still work but start the warm-up for better deliverability.
                        </p>
                      </div>
                    )}
                </CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>
                    Select the verified email address that will appear as the sender
                  </CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex space-x-1 rounded-lg bg-slate-100 p-1">
                    {(["database", "custom"] as const).map((src) => (
                      <button
                        key={src}
                        type="button"
                        onClick={() => setRecipientSource(src)}
                      ...
                      >
                        {src === "database" ? "Database Recipients" : "Custom Emails"}
                      </button>
                    ))}
                  </div>
                      ...
                      <div className="max-h-48 overflow-y-auto border border-slate-200 rounded-lg p-2">
                        {recipients.length === 0 && selectedRecipients.length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-6 gap-3 text-center">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400 mt-1">
                                Use "Select from Users" above to pick recipients from your user base.
                              </p>
                            </div>
                            <Button
                              type="button"
                          ...
                          <div className="space-y-1.5">
                            ...
                            {/* Show user-picker selections not in warm-up DB */}
                            {selectedRecipients
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div>
                    <Label className="text-sm font-semibold">Custom Subject</Label>
                    <Input
                  ...
                  <div>
                    <Label className="text-sm font-semibold">Custom Body</Label>
                    <Textarea
              ...
              <Card className="bg-blue-50 border-blue-200">
                <CardContent className="pt-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="font-semibold text-blue-900 text-sm">Send Summary</h4>
                  </div>
                  <div className="grid grid-cols-2 gap-y-1.5 text-sm">
                    ...
                    <span className="font-medium">{resolvedFromEmail || "Not set"}</span>
