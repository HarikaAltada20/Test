...
export default function AdminBlogsCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (thumbnailFile) {
        ...
        const fileName = `blog_thumbnails/${timestamp}.${fileExt}`;

        // Use the same bucket pattern as contests (contest-assets)
      ...
      // After creation, go back to drafts list
      router.push("/dashboard/admin/blogs");
    } catch (error: any) {
      toast.error(
        error?.message || "An unexpected error occurred while creating blog"
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8 pb-4 sm:pb-6 lg:pb-8 px-2 sm:px-4 lg:px-0">
      {/* Header: match admin create/management pages */}
      ...
      {/* Main form card, styled like other admin cards */}
      <Card
        className={cn(
