...
export default async function AdminContestDetailPage({
  ...
  try {
    ...
    if (isTwitterCampaign) {
      ...
      // Include all tweets (including rejected) so admins can see and change their status
      ...
      if (tweetsError && tweetsError.code === '42703') {
        console.log(`[Admin page.tsx] Some columns don't exist, fetching without them`);
      ...
      if (tweetsError) {
        ...
        twitterTweetsData = (tweetsData || []).map((tweet: any) => ({
          ...
          created_at: tweet.first_fetched_at || tweet.last_updated_at || tweet.tweet_created_at, // Use first_fetched_at as created_at
        }));
        console.log(
          `[Admin page.tsx] Fetched ${twitterTweetsData.length} Twitter tweets for contest ${contestId}`,
          twitterTweetsData.length > 0 ? `Sample tweet: ${JSON.stringify(twitterTweetsData[0], null, 2)}` : "No tweets found"
        );
      }
    } else {
      console.log(`[Admin page.tsx] Not a Twitter campaign - skipping Twitter tweets fetch`);
    }
    ...
    if (isTwitterCampaign) {
      try {
        ...
        console.error(`[Admin page.tsx] Error fetching creator leaderboard data:`, error);
      }
    }
      ...
      ? twitterTweetsData.map((tweet: any) => {
        ...
        if (creatorProfile?.twitter_account) {
          try {
            ...
            creatorDisplayName = twitterAccount?.name || twitterAccount?.username;
            creatorUsername = twitterAccount?.username || tweet.twitter_username;
            creatorAvatarUrl = twitterAccount?.profile_picture_url;
          } catch (e) {
            console.error("[Admin page.tsx] Error parsing Twitter account JSON:", e);
          }
        }
        ...
        if (!creatorUsername) {
          creatorUsername = user?.username || tweet.twitter_username || "Unknown User";
        }
    ...
    console.log(
      `[Admin page.tsx] Mapped submissions for contest ${contestId}:`,
      {
        ...
        platform: contestData.platform,
        contest_format: contestData.contest_format,
        sampleTwitterSubmission: twitterSubmissions.length > 0 ? twitterSubmissions[0] : null,
      }
    );
