...
export default async function ContestsPage() {
  ...
  const contestsWithCalculatedBudgets = await Promise.all(
    (contestsData || []).map(async (contest) => {
      ...
      console.log(
        `[Twitter CPM Budget] Checking contest ${contest.id} (${contest.title}):`,
        {
      ...
      if (
        ...
        console.log(
          `[Twitter CPM Budget] Processing Twitter CPM contest ${contest.id} (${contest.title})`
        );
        ...
        console.log(
          `[Twitter CPM Budget] All tweets for contest ${contest.id}:`,
          allTwitterTweets?.slice(0, 2)
        );
        ...
        console.log(
          `[Twitter CPM Budget] Fetched ${
            twitterTweets?.length || 0
          } Twitter tweets for contest ${contest.id}:`,
          twitterTweets?.slice(0, 2)
        );
        ...
        console.log(
          `[Twitter CPM Budget] Mapped ${submissions.length} submissions for budget calculation:`,
