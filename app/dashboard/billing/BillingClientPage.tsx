    ...
    return (
        <div className="container mx-auto py-8 px-4 md:px-6">
            <div className="flex items-center justify-between mb-8">
                <h1 className="text-3xl font-bold">Billing & Account</h1>
            </div>
            ...
            <Tabs defaultValue="cash" className="w-full" onValueChange={(value) => setActiveTab(value as 'cash' | 'coins')}>
                ...
                <TabsContent value="cash">
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium">Contests Run</CardTitle>
                                <BarChart3 className="h-4 w-4 text-muted-foreground" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold">{profile.total_contests_run}</div>
                                <p className="text-xs text-muted-foreground">Total contests created</p>
                            </CardContent>
                        </Card>
                                                ...
                                                <TableCell>
                                                    {req.status === 'pending' && (
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            onClick={() => handleCancelWithdrawal(req.id, req.amount, req.amount_type)}
                                    ...
                                    {coinTransactions.length === 0 ? (
                                        ...
                                        coinTransactions.map((transaction) => (
                                            <TableRow key={transaction.id}>
                                                ...
                                                <TableCell className={transaction.coins > 0 ? "text-green-600" : "text-red-600"}>
                                                    {transaction.coins > 0 ? "+" : ""}{formatCoins(transaction.coins)}
                                                </TableCell>
                                                <TableCell>
                                                    <Badge variant={
                                            ...
                                            <TableRow key={req.id}>
                                                ...
                                                <TableCell className="max-w-xs truncate">
                                                    {req.redeemed_item_description ? (
                                                        typeof req.redeemed_item_description === 'string' ? req.redeemed_item_description :
                                                            (req.redeemed_item_description as any)?.name || JSON.stringify(req.redeemed_item_description)
                                                    ) : "N/A"}
                                                </TableCell>
                                                <TableCell className="max-w-xs truncate">{req.user_notes || "N/A"}</TableCell>
                                                <TableCell>
            ...
            <Dialog open={isPayoutModalOpen} onOpenChange={(isOpen) => {
            ...
            <Dialog open={isWithdrawModalOpen} onOpenChange={(isOpen) => {
                ...
                <DialogContent className="sm:max-w-[425px]">
                    ...
                    <div className="py-4 space-y-4">
                        ...
                        {activeTab === 'coins' && (
                            <>
                                <div className="text-lg">
                                    Available Coins: <span className="font-semibold">{formatCoins(userData?.coins || 0)}</span>
                                </div>
                                <div>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => setWithdrawAmountCoins(parseInt(e.target.value, 10) || 0)}
                        ...
                        {payoutMethods.length === 0 && (
                            <p className="text-sm text-red-500">You have no payout methods. Please add one first.</p>
                        )}
                    </div>
                    <DialogFooter>
                        <DialogClose asChild>
