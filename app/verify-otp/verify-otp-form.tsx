...
export function VerifyOtpForm() {
    ...
    const handleResendCode = async () => {
        ...
        try {
            ...
            toast({ title: "Code Resent", description: "A new verification code has been sent.", duration: 5000 })
            setResendCooldown(60);
            startCooldownTimer();
        } catch (err: any) {
            setError(`Resend failed: ${err.message}`);
            toast({ variant: "destructive", title: "Resend Failed", description: err.message || "Could not send code.", duration: 5000 })
        } finally {
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            localStorage.setItem('verificationAttempt', JSON.stringify({ email, timestamp: new Date().toISOString() }));
            const { data, error: verifyError } = await supabase.auth.verifyOtp({ email, token: otp, type: 'signup' });
            if (verifyError) throw verifyError;
            if (!data.session || !data.user) throw new Error("Verification successful, but no session created.");
            ...
            toast({ variant: "destructive", title: "Verification Failed", description: err.message || "Failed to verify OTP.", duration: 5000 })
        } finally {
            setIsLoading(false);
        }
    }
            ...
            <div className="w-full max-w-md">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-8">
                    ...
                    <div className="mt-4 text-center">
                        ...
                        >
                            {isResending ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <RefreshCw className="h-4 w-4 mr-1" />}
