  ...
  const finishWithToast = useCallback(
    (job: BulkModerationJobStatus, meta: TrackedJob) => {
      ...
      if (job.status === "completed") {
        let description = `${success} of ${total} submission(s) were ${actionText}. Processed ${processed} · Success ${success} · Failed ${failed}.`;
        if (meta.action === "verified" && meta.qualityScore != null) {
