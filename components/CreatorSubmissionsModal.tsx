...
export function CreatorSubmissionsModal({
    ...
    const [statusFilter, setStatusFilter] = useState<'all' | 'verified_or_paid' | 'pending' | 'verified' | 'rejected' | 'paid'>('all');
    const [sortBy, setSortBy] = useState<'views-desc' | 'views-asc' | 'date-desc' | 'date-asc'>('date-desc');
    ...
    const handleBulkPayment = async (type: 'standard' | 'bonus' | 'both', isBulkTransaction: boolean) => {
        const selectedIds = Array.from(selectedSubmissions);
        ...
        if (verifiedSubs.length === 0) {
            alert('No verified submissions selected. Only verified submissions can be paid.');
            return;
        }
        ...
        if (isBulkTransaction) {
            ...
            try {
                ...
                if (data.cap_reached) {
                    ...
                    message.push(`Remaining cap: $${(data.remaining_cap / 100).toFixed(2)}`);
                }
    ...
    const filteredSubmissions = submissions.filter((sub) => {
        ...
        if (statusFilter === 'verified_or_paid') return sub.status === 'verified' || sub.status === 'paid';
    ...
    const statusCounts = {
        ...
        verifiedOrPaid: submissions.filter(s => s.status === 'verified' || s.status === 'paid').length,
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-[98vw] max-h-[98vh] p-0 gap-0" hideCloseButton onOpenAutoFocus={(e) => e.preventDefault()}>
                <div className="flex flex-col h-[98vh] overflow-hidden">
                    ...
                    {/* Status Filter Tabs */}
                    <div className="flex items-center justify-between px-6 py-3 border-b bg-white dark:bg-gray-950 flex-shrink-0">
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <Button
                                size="sm"
                                variant={statusFilter === 'verified_or_paid' ? 'default' : 'outline'}
                                onClick={() => setStatusFilter('verified_or_paid')}
                                className={statusFilter === 'verified_or_paid' ? 'bg-purple-600 hover:bg-purple-700' : ''}
                            >
                                Verified + Paid <Badge variant="secondary" className="ml-2">{statusCounts.verifiedOrPaid}</Badge>
                            </Button>
                            <Button
                                size="sm"
                                variant={statusFilter === 'pending' ? 'default' : 'outline'}
                                onClick={() => setStatusFilter('pending')}
                                className={statusFilter === 'pending' ? 'bg-purple-600 hover:bg-purple-700' : ''}
                            >
                            ...
                            <Button
                                ...
                                className={statusFilter === 'verified' ? 'bg-purple-600 hover:bg-purple-700' : ''}
                            >
                                Verified <Badge variant="secondary" className="ml-2">{statusCounts.verified}</Badge>
                            </Button>
                            <Button
                                size="sm"
                                variant={statusFilter === 'rejected' ? 'default' : 'outline'}
                                onClick={() => setStatusFilter('rejected')}
                                className={statusFilter === 'rejected' ? 'bg-purple-600 hover:bg-purple-700' : ''}
                            >
                            ...
                            <Button
                                ...
                                className={statusFilter === 'paid' ? 'bg-purple-600 hover:bg-purple-700' : ''}
                            >
                                Paid <Badge variant="secondary" className="ml-2">{statusCounts.paid}</Badge>
                            </Button>
                        </div>
                    ...
                    <div className="flex-1 overflow-auto px-6 py-4">
                        <Table>
                            <TableHeader className="sticky top-0 bg-white dark:bg-gray-950 z-20 shadow-sm">
                                <TableRow className="border-b-2">
                                    ...
                                    <TableHead className="bg-white dark:bg-gray-950 min-w-[200px]">Content</TableHead>
                                    <TableHead className="text-center bg-white dark:bg-gray-950">Views</TableHead>
                                    <TableHead className="text-center bg-white dark:bg-gray-950">Likes</TableHead>
                                    <TableHead className="text-center bg-white dark:bg-gray-950">Comments</TableHead>
                                    <TableHead className="text-center bg-white dark:bg-gray-950">Expected Reward</TableHead>
                                    <TableHead className="text-center bg-white dark:bg-gray-950">Reward Granted</TableHead>
                                    {hasBonus && (
                                        <>
                                            <TableHead className="text-center bg-white dark:bg-gray-950">Bonus Expected</TableHead>
                                            <TableHead className="text-center bg-white dark:bg-gray-950">Bonus Granted</TableHead>
                                        </>
                                    )}
                                    <TableHead className="bg-white dark:bg-gray-950">Status</TableHead>
                                    <TableHead className="bg-white dark:bg-gray-950 min-w-[180px]">Submitted</TableHead>
                                    <TableHead className="text-center bg-white dark:bg-gray-950">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {sortedSubmissions.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={hasBonus ? 13 : 11} className="text-center py-8 text-gray-500">
                                            No submissions found for this filter.
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    sortedSubmissions.map((submission, index) => {
                                        const likes = submission.other_stats?.youtube?.likes || submission.other_stats?.instagram?.likes || 0;
                                        const comments = submission.other_stats?.youtube?.comments || submission.other_stats?.instagram?.comments || 0;
                                        ...
                                        const expectedBonus = ((submission.status === 'verified' || submission.status === 'paid') && hasBonus) ? flatFeeBonus : 0;
                                        ...
                                        return (
                                            <TableRow key={submission.id}>
                                                ...
                                                <TableCell className="text-center font-mono">{likes.toLocaleString()}</TableCell>
                                                <TableCell className="text-center font-mono">{comments.toLocaleString()}</TableCell>
                                                ...
                                                {hasBonus && (
                                                    <>
                                                        <TableCell className="text-center font-medium">
                                                            {expectedBonus > 0 ? formatCurrency(expectedBonus) : '-'}
                                                        </TableCell>
                                                        <TableCell className="text-center font-medium text-green-600">
                                                            {grantedBonus > 0 ? formatCurrency(grantedBonus) : '-'}
                                                        </TableCell>
                                                    </>
                                                )}
                                                <TableCell>
                                                ...
                                                <TableCell className="text-right">
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            ...
                                                            {submission.status === 'verified' && !submission.paid && (
                                                                <>
                                                                    <DropdownMenuSeparator />
                                                                    ...
                                                                    {hasBonus && !submission.bonus_paid && (
                                                                        <>
                                                                            <DropdownMenuItem onClick={() => onPayment(submission.id, 'bonus')}>
                                                                                <DollarSign className="h-4 w-4 mr-2" />
                                                                                Mark Bonus as Paid
                                                                            </DropdownMenuItem>
                                                                            <DropdownMenuItem onClick={() => onPayment(submission.id, 'both')}>
                                                                                <DollarSign className="h-4 w-4 mr-2" />
                                                                                Mark Both as Paid
                                                                            </DropdownMenuItem>
                                                                        </>
                                                                    )}
                                                                    {hasBonus && !submission.bonus_paid && submission.paid && (
                                                                        <DropdownMenuItem onClick={() => onPayment(submission.id, 'bonus')}>
                                                                            <DollarSign className="h-4 w-4 mr-2" />
                                                                            Mark Bonus as Paid
                                                                        </DropdownMenuItem>
