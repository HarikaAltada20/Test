...
export function PaymentAnimation({
    ...
    const [stage, setStage] = useState<'processing' | 'result' | 'complete'>('processing');
    ...
    return (
        <div className={cn(
            ...
            <div className="relative flex flex-col items-center justify-center bg-white dark:bg-gray-900 rounded-3xl p-8 shadow-2xl max-w-sm w-full mx-4 border border-gray-100 dark:border-gray-800">
                ...
                <div className="relative z-10 flex flex-col items-center space-y-6 w-full">
                    ...
                    <div className="text-center space-y-3 w-full">
                        {stage === 'processing' && (
                            <>
                                ...
                                {amount && (
                                    <div className="mt-3 px-4 py-2 bg-gray-50 dark:bg-gray-800 rounded-xl">
                        ...
                        {(stage === 'result' || stage === 'complete') && !isSuccess && (
                            <>
                                ...
                                {error && (
                                    <div className="mt-3 px-4 py-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl">
                        ...
                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                            <div
                                className={cn(
                                    ...
                                    stage === 'result' && isSuccess && "bg-green-500 dark:bg-green-400",
                                    stage === 'result' && !isSuccess && "bg-red-500 dark:bg-red-400",
                                    stage === 'complete' && isSuccess && "bg-green-500 dark:bg-green-400",
                                    stage === 'complete' && !isSuccess && "bg-red-500 dark:bg-red-400"
                                )}
                                style={{
                    ...
                    <div className="text-center">
                        <p className={cn(
                            ...
                            stage === 'result' && isSuccess && "text-green-600 dark:text-green-400",
                            stage === 'result' && !isSuccess && "text-red-600 dark:text-red-400",
                ...
                {stage === 'result' && isSuccess && (
                    <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-3xl">
                        ...
                        <div className="absolute top-8 right-8 w-1 h-1 bg-green-500 rounded-full opacity-40 animate-pulse" style={{ animationDelay: '0.5s' }} />
                        <div className="absolute bottom-6 left-6 w-1.5 h-1.5 bg-green-400 rounded-full opacity-25 animate-pulse" style={{ animationDelay: '1s' }} />
                    </div>
                )}
            </div>
        </div>
...
export function PaymentSuccessAnimation(props: Omit<PaymentAnimationProps, 'type'>) {
    return <PaymentAnimation {...props} type="success" />;
}

// New Failure Animation
export function PaymentFailureAnimation(props: Omit<PaymentAnimationProps, 'type'>) {
...
export function PaymentSuccessCheck({ className }: { className?: string }) {
    return (
        <div className={cn("flex items-center space-x-2 text-green-600 dark:text-green-400", className)}>
...
export function PaymentFailureCheck({ className }: { className?: string }) {
    return (
        <div className={cn("flex items-center space-x-2 text-red-600 dark:text-red-400", className)}>
