...
function expandCountryLabel(label: string): {
  primary: string;
  short?: string;
} {
...
function expandGenderLabel(label: string): { primary: string; short?: string } {
  ...
  const full =
    GENDER_EXPAND[t] ?? GENDER_EXPAND[t.toUpperCase()] ?? GENDER_EXPAND[key];
  if (full) {
    const short =
...
function DemographicBarList({
  ...
  if (!rows.length) {
    return (
      <p
        className={cn(
          "text-xs py-6 text-center rounded-xl border border-dashed",
          isDark
...
function BreakdownBlock({
  ...
  if (!breakdowns.length) {
    return (
      <span
        className={cn("text-xs", isDark ? "text-slate-400" : "text-slate-600")}
      >
        No breakdown rows
      </span>
    );
          ...
          {breakdowns.flatMap((b, bi) =>
            (b.results ?? []).map((r, ri) => (
              ...
              >
                ...
                <td
                  className={cn(
                    "p-1.5 pr-2 text-right tabular-nums font-medium",
...
function DemographicsSection({
  ...
  const tf = normalizeDemographicsTimeframe(bundle.timeframe).replace(/_/g, " ");
  ...
  return (
    ...
    >
      ...
      {bundle.note && (
        <p
          className={cn(
            "text-[11px] leading-relaxed border-l-2 border-[#E1306C]/50 pl-2",
            isDark ? "text-slate-400" : "text-slate-600",
          )}
        >
          {bundle.note}
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(["country", "age", "gender"] as const).map((d) => (
              ...
              >
                {d === "country" ? "Country" : d === "age" ? "Age" : "Gender"}
              </button>
            ))}
          </div>
          ...
          <div>
            ...
            {hasFollowerRows && !hasEngagedRows && audience === "engaged" && (
              <p
                className={cn(
                  "text-xs rounded-lg border border-dashed p-2 mb-3",
                  isDark
                    ? "text-slate-400 border-white/15 bg-black/10"
        ...
        {metricKeys.map((k) => {
          ...
          return (
            ...
            >
              ...
              {isBreakdownObj && (
                <div className="mt-2">
                  <BreakdownBlock data={v as BreakdownData} isDark={isDark} />
                </div>
              )}
            </div>
          );
  ...
  return (
    ...
    >
      ...
      >
        ...
        below are separate estimates per bucket and won’t add up to this number.
      </p>
      <div
        className={cn(
          ...
          <tbody>
            {rows.map((r) => (
              ...
              >
                ...
                <td
                  className={cn(
                    "p-2 text-right tabular-nums [&_a]:tabular-nums",
...
export type InstagramCreatorAnalyticsModalProps = {
  ...
  /** Called when archive cache updates so export/submissions stay in sync. */
  onArchiveUpdated?: (creatorId: string, archive: unknown) => void;
};
...
export function InstagramCreatorAnalyticsModal({
    ...
    async (forceRefresh: boolean) => {
      ...
      try {
        ...
        if (data.archive != null) {
          onArchiveUpdatedRef.current?.(creatorId, data.archive);
        }
      } finally {
        setLoading(false);
  ...
  const metricKeys = entry?.metrics
    ? sortMetricKeys(Object.keys(entry.metrics))
    : [];
          ...
          <div className="space-y-4 pt-1">
            ...
            {!loading &&
              ...
              !entry?.demographics && (
                ...
                >
                  No metric values returned for this window (Meta may return
                  empty data for small accounts or delayed processing). Try
                  Refresh from Meta or pick another range.
                </p>
              )}
          </div>
