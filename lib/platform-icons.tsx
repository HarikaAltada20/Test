...
export const getPlatformIcon = (
  ...
  size: "sm" | "md" | "lg" = "md",
  /** Override brand fills (e.g. `"currentColor"` for themed tabs). */
  fill?: string,
) => {
  ...
  const youtubeFill = fill ?? "#FF0000";
  const tiktokFill = fill ?? "#000000";
  const twitterFill = fill ?? "#1DA1F2";
  const linkedinFill = fill ?? "#0077B5";
  const useThemedFill = Boolean(fill);
    ...
    return (
      <span className={`inline-flex items-center justify-center ${sizeClasses[size]}`}>
        <svg viewBox="0 0 24 24" className={sizeClasses[size]}>
          ...
          <path
            fill={useThemedFill ? fill! : "url(#instagram-gradient)"}
...
export const getPlatformIconWithFallback = (
  platform?: string | null,
  size: "sm" | "md" | "lg" = "md",
) => {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center ${sizeClasses[size]} text-gray-600`}
    >
      <svg
        viewBox="0 0 24 24"
        className={sizeClasses[size]}
