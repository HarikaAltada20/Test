...
export type ListEmailCampaignsPaginatedParams = {
  projectId?: string | null;
  status?: string | null;
  search?: string | null;
  page?: number;
...
export type PaginatedEmailCampaignsResult = {
...
async function attachCampaignListStats(
  rows: CampaignRow[],
...
export async function listEmailCampaignsPaginated(
...
): Promise<PaginatedEmailCampaignsResult> {
  const db = createAdminClient();
  const page = Math.max(1, params.page ?? 1);
  const limit = Math.min(
    100,
    Math.max(1, params.limit ?? 25),
  );
  ...
  query = applyCampaignListFilters(query, params);
  ...
  let activeQuery = db
    ...
    .eq("status", "active");
  activeQuery = applyCampaignListFilters(activeQuery, params);
  const { count: activeCount } = await activeQuery;

  const campaigns = await attachCampaignListStats(rows);
