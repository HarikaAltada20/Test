...
-- }
--
-- This migration is atomic:
--   1) Drop campaign_list_* / contest_ids_matching_user_countries (depend on view)
--   2) ALTER max_earnings_per_creator → jsonb + recreate contests_with_status
--   3) Restore contest_ids_matching_user_countries
--   4) Restore campaign_list_* with CSV platform token matching
--
...
-- Restore opportunity country filter helper (dropped above).
...
-- ---------------------------------------------------------------------------
-- Campaign list platform token filter + restore campaign_list_* (formerly
-- db/migrations/20260908130000_campaign_list_platform_token_filter.sql).
