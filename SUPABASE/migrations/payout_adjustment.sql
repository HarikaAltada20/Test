-- Payout Adjustment: schema + view updates
-- Safe to run in Supabase SQL editor or via migration.

-- 1) Add new columns on contests (idempotent)
-- numeric(5,2) allows fractional % e.g. 13.36
ALTER TABLE public.contests
ADD COLUMN IF NOT EXISTS payout_adjustment_percentage numeric(5,2),
ADD COLUMN IF NOT EXISTS payout_adjustment_mode text;

-- If you ran the old script (integer), convert to numeric so 13.36% works
ALTER TABLE public.contests
ALTER COLUMN payout_adjustment_percentage TYPE numeric(5,2);

-- 2) Recreate contests_with_status view to include new fields
...
'View that includes all contest columns (including new features: multiple submissions, content type, bonus details, earnings cap, payout adjustment) plus computed status field for easy querying';
