...
AS $$
  SELECT
    CASE
      WHEN p_dual_rewards_payout IS NOT NULL
        AND jsonb_typeof(p_dual_rewards_payout) = 'object'
        AND (p_dual_rewards_payout ? 'cpm_cents')
        AND (p_dual_rewards_payout ? 'milestone_cents')
      THEN GREATEST(0, COALESCE((p_dual_rewards_payout->>'cpm_cents')::bigint, 0))
      WHEN COALESCE(p_paid, false) THEN GREATEST(0, COALESCE(p_earnings, 0))
      ELSE 0::bigint
    END,
    CASE
      WHEN p_dual_rewards_payout IS NOT NULL
        ...
        AND (p_dual_rewards_payout ? 'cpm_cents')
        AND (p_dual_rewards_payout ? 'milestone_cents')
      THEN GREATEST(0, COALESCE((p_dual_rewards_payout->>'milestone_cents')::bigint, 0))
      WHEN COALESCE(p_bonus_paid, false) THEN GREATEST(0, COALESCE(p_bonus_amount, 0))
      ELSE 0::bigint
    END;
$$;
